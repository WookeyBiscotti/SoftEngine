...
int main() {
	...
	auto mouseId = world.addPoint(0, 0, soften::PointFlags::STATIC);
	auto mouseCId = world.addConstrain(mouseId, mouseId);
	world.c(mouseCId).flags = soften::ConstrainFlags::DISABLE;
	...
	while (running) {
		...
		while (window.pollEvent(event)) {
			if (event.type == sf::Event::EventType::Closed) {
				...
				world.c(mouseCId).flags = soften::ConstrainFlags::NONE;
				...
				if (!world.c(mouseCId).is(soften::ConstrainFlags::DISABLE)) {
					auto rawPos = window.mapPixelToCoords(sf::Mouse::getPosition(window));
					soften::Vec2 mouse = {rawPos.x, rawPos.y};
					world.p(mouseId).p2 = mouse;
				}
		...
		for (const auto& c : world.constrains()) {
			if (c.is(soften::ConstrainFlags::DISABLE)) {
				continue;
			}
			sf::Vertex line[] = {sf::Vertex(sf::Vector2f(world.points()[c.i].p2.x, world.points()[c.i].p2.y)),
